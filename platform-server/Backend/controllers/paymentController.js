...
export const buySubscription = catchAsyncError(async(req, res, next)=>{
    ...
    const plan_id = process.env.PLAN_ID || "plan_MstwrrYJM9paCI"
...
export const cancelSubscription = catchAsyncError(async(req, res, next)=>{
    ...
    await instance.subscriptions.cancel(subscriptionId);
     ...
     const refundTime = process.env.REFUND_DAYS * 24 * 60 * 60 
     if(refundTime > gap){
    ...
    res.status(200).json({
        success: true,
        message:
        refund?"Subscription cancelled, You will get your refund within 7 days."
